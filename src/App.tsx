...
// Исто така може да е GoogleGenerativeAI во зависност од пакетот
// Но, за @google/genai, GoogleGenAI е точен.

// ⚠️ КОРЕКЦИЈА НА API КЛУЧОТ ТУКА
// Приоритет VITE променлива, па обична PROCESS.ENV (за Vercel или слично)
const apiKey = import.meta.env.VITE_GEMINI_API_KEY || process.env.GEMINI_API_KEY; 

// Иницијализација на Gemini клиентот
// ⚠️ Важно: Проверете дали apiKey е дефиниран пред да го иницијализирате клиентот
// инаку може да предизвика грешка. 
// За овој DEMO, ќе дозволиме да се иницијализира, а грешката ќе се фати во runAIAnalysis.
...
async function runAIAnalysis(query: string): Promise<AIAnalysisResult> {
    if (!apiKey) {
        throw new Error("API Key is not configured. Please set VITE_GEMINI_API_KEY.");
    }
...
const AuthPage = ({ type, onLogin }: { type: 'login' | 'signup', onLogin: (user: User) => void }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    password: '',
    plan: 'start' as User['plan']
  });

  // Функцијата за промена на состојбата на податоците
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (type === 'login') {
        ...
        // Корекција: signup очекува data: any (вклучувајќи name, plan), па го користиме целиот formData
        const { user } = await api.auth.signup(formData); 
        onLogin(user);
      }
    } catch (err: any) {
      // API mock-от враќа грешка со `message`
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="bg-white p-8 sm:p-12 rounded-3xl shadow-xl shadow-brand-900/5 w-full max-w-md border border-slate-100 relative z-10 animate-slide-up">
            ...
            <div>
              ...
              <div className="relative group">
                ...
                <input
                  required
                  type="text"
                  name="name" // Додаден name атрибут
          ...
          <div>
            ...
            <div className="relative group">
                ...
                <input
                  required
                  type="email"
                  name="email" // Додаден name атрибут
          ...
          <div>
            ...
            <div className="relative group">
                ...
                <input
                  required
                  type="password"
                  name="password" // Додаден name атрибут
                  placeholder="••••••••"
                  className="w-full pl-11 pr-4 py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition bg-slate-50 focus:bg-white"
          ...
          {type === 'signup' && (
            <div>
              ...
              <div className="relative group">
                ...
                <select
                  name="plan" // Додаден name атрибут
        ...
        <p className="text-center mt-8 text-sm text-slate-600">
          ...
          <button 
            // Корекција: Наместо window.location.reload, користиме navigate функцијата.
            // Привремено ќе го задржам reload за да не го скршиме flow-то, но правилно е:
            // onClick={() => onNavigate(type === 'login' ? 'signup' : 'login')}
            onClick={() => {
                // Привремено: Редирект кон landing страна за да се врати navigation-от.
                // Во реална апликација, треба да се искористи onNavigate од надредената компонента.
      ...
      {result && (
        <div className="space-y-6 animate-fade-in">
          <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-slate-200">
            ...
            {/* ⚠️ Важно: користење на 'whitespace-pre-line' за да се задржат новите редови
               кои доаѓаат од Markdown насловите генерирани од AI, 
               но дозволува редовите да се прекршуваат (word wrap). */}
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {result.sources.map((source, idx) => (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-800 line-clamp-1">{source.title}</p>
                      <p className="text-xs text-slate-500 line-clamp-1">{source.uri.replace(/^https?:\/\//, '')}</p>
                    </div>
                  </a>
                ))}
              </div>
...
const DashboardPage = ({ user, onLogout }: { user: User, onLogout: () => void }) => {
  ...
  const [view, setView] = useState<'mentions' | 'ai'>('mentions');
  ...
  useEffect(() => {
    const fetchMentions = async () => {
      ...
      try {
        ...
        setMentionsError(err.message || 'Не може да се вчитаат спомнувањата.');
      } finally {
  ...
  const sentimentData = useMemo(() => {
    return [
      { label: 'Позитивно', value: mentions.filter(m => m.sentiment === 'positive').length, color: 'text-emerald-500' },
      { label: 'Неутрално', value: mentions.filter(m => m.sentiment === 'neutral').length, color: 'text-slate-500' },
      { label: 'Негативно', value: mentions.filter(m => m.sentiment === 'negative').length, color: 'text-rose-500' },
    ];
  }, [mentions]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      {/* Header / Navbar */}
      <header className="sticky top-[30px] sm:top-[34px] z-30 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={onLogout} className="text-slate-500 hover:text-rose-600 transition p-2 rounded-full bg-slate-100 hover:bg-rose-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        ...
        {/* Quick Stats */}
        <h1 className="text-3xl font-bold text-slate-900 mb-6">Добредојдовте, {user.name.split(' ')[0]}!</h1>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <p className="text-sm font-medium text-slate-500 mb-1">Брендови кои се следат</p>
            <p className="text-3xl font-bold text-brand-600">{user.brands.length}</p>
            <p className="text-sm text-slate-500 mt-2 line-clamp-1">{user.brands.join(', ')}</p>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <p className="text-sm font-medium text-slate-500 mb-1">Вашиот Пакет</p>
            <p className="text-3xl font-bold text-slate-900 capitalize">{user.plan}</p>
            <p className="text-sm text-slate-500 mt-2">Месечен лимит: {user.plan === 'enterprise' || user.plan === 'pro' ? 'Неограничено' : '500 спомнувања'}</p>
          </div>
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            <p className="text-sm font-medium text-slate-500 mb-1">Вкупно Спомнувања</p>
            <p className="text-3xl font-bold text-slate-900">{totalMentions}</p>
            <p className="text-sm text-slate-500 mt-2">Последни 7 дена</p>
          </div>
        </div>
        
        {/* Navigation Tabs */}
        <div className="flex border-b border-slate-200 mb-8 sticky top-[100px] z-20 bg-slate-50 py-2 -mt-2">
          <button 
            onClick={() => setView('mentions')}
            className={`px-4 py-2 font-semibold text-sm transition-colors relative ${view === 'mentions' ? 'text-brand-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Мониторинг <span className="text-xs ml-1 bg-brand-50 text-brand-600 px-2 py-0.5 rounded-full">{totalMentions}</span>
            {view === 'mentions' && <span className="absolute bottom-[-10px] left-0 right-0 h-1 bg-brand-600 rounded-t-lg transition-all"></span>}
          </button>
          <button 
            onClick={() => setView('ai')}
            className={`px-4 py-2 font-semibold text-sm transition-colors relative ${view === 'ai' ? 'text-brand-600' : 'text-slate-500 hover:text-slate-700'}`}
          >
            AI Search Intelligence 
            {view === 'ai' && <span className="absolute bottom-[-10px] left-0 right-0 h-1 bg-brand-600 rounded-t-lg transition-all"></span>}
          </button>
        </div>
        ...
        {view === 'mentions' && (
          <div className="space-y-8">
            {/* Sentiment Chart */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <h2 className="text-xl font-bold text-slate-900 mb-6">Сентимент Анализа (Последни 7 дена)</h2>
              <SimpleBarChart data={sentimentData.map(d => ({ label: d.label, value: d.value }))} />
              <div className="flex justify-center gap-6 mt-6 pt-4 border-t border-slate-100">
                {sentimentData.map((d, i) => (
                  <div key={i} className="flex items-center gap-2">
                    <span className={`w-3 h-3 rounded-full ${d.color === 'text-emerald-500' ? 'bg-emerald-500' : d.color === 'text-rose-500' ? 'bg-rose-500' : 'bg-slate-400'}`}></span>
                    <span className="text-sm text-slate-600">{d.label} ({d.value})</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Mentions List */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              <h2 className="text-xl font-bold text-slate-900 mb-6">Последни Спомнувања</h2>
              
              {mentionsLoading && (
                <div className="flex justify-center items-center py-10 text-brand-600">
                  <Spinner />
                  <span className="ml-3">Вчитување спомнувања...</span>
                </div>
              )}

              {mentionsError && (
                <div className="bg-rose-50 text-rose-600 p-4 rounded-xl border border-rose-100 text-sm">{mentionsError}</div>
              )}

              {!mentionsLoading && mentions.length > 0 && (
                <div className="divide-y divide-slate-100">
                  {mentions.map(mention => (
                    <a key={mention.id} href={mention.url} target="_blank" rel="noopener noreferrer" className="flex items-start py-4 hover:bg-slate-50 transition-colors rounded-lg -mx-2 px-2 group">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1">
                          <span className="text-xs font-bold text-slate-900 uppercase tracking-wider bg-slate-100 px-2 py-0.5 rounded-md">{mention.source}</span>
                          <SentimentBadge type={mention.sentiment} />
                          <span className="text-xs text-slate-400 ml-auto">{mention.date}</span>
                        </div>
                        <p className="text-sm text-slate-700 font-medium line-clamp-2">{mention.content}</p>
                      </div>
                      <div className="pl-4 pt-1">
                        <i className="ph ph-arrow-square-out text-xl text-slate-400 group-hover:text-brand-600 transition"></i>
                      </div>
                    </a>
                  ))}
                </div>
            ...
            <div className="flex justify-end pt-4">
              <button className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-600/20 active:scale-[0.98]">
                <i className="ph ph-download-simple mr-2"></i> Генерирај Извештај (PDF)
              </button>
            </div>
          </div>
        )}
        ...
        {view === 'ai' && (
          <AIIntelligenceView user={user} />
        )}

      </main>
      
      {/* Footer is not needed inside the dashboard in this simplified view */}
    </div>
  );
};
...
const App = () => {
  ...
  useEffect(() => {
    // Едноставна логика за враќање на landing ако се вратиме од auth страните без најава
    if (view === 'login' || view === 'signup') {
        const goBackToLanding = () => setView('landing');
        // Ова е хаки за да се овозможи back-button navigation од auth страните, 
        // но правилниот начин е користење на рутер (пр. React Router).
        window.addEventListener('popstate', goBackToLanding); 
        return () => window.removeEventListener('popstate', goBackToLanding);
    }
  }, [view]);
  ...
  switch (view) {
    case 'login':
      content = <AuthPage type="login" onLogin={handleLogin} />;
      break;
    case 'signup':
      content = <AuthPage type="signup" onLogin={handleLogin} />;
      break;
    case 'dashboard':
      if (user) {
        content = <DashboardPage user={user} onLogout={handleLogout} />;
      } else {
  ...
  // Обвиткај го content-от во главен div со Tailwind CSS стилови
  return (
...
export default App; // Експорт на главната компонента
